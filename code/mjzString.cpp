...
mjz_Str ULL_LL_to_str(uint64_t value, int radix, bool is_signed,
  ...
  ret_var.addto_length(
    strlen(b_U_lltoa(value, (char *)ret_var, radix, is_signed, force_neg)), 1);
  return ret_var;
}
...
long long C_STR_to_LL(const char *buffer, uint8_t buffer_len, int radix,
  ...
  while (WHILE_Index_for_buffer < buffer_len) {
    ...
    if (is_neg_bool_buf) {
      ...
      volatile bool HOLDER_does_not_do_any_thing = 0;
  ...
  uint while_2_i_max = WHILE_Index_for_int_buf + 1;
...
bool mjz_Str::reserve(unsigned int size_, bool just_size) {
  ...
  if (!just_size && different_of_size_and_cap >= 0) {
    long long minimumcapadd = min(capacity / 5 , stack_buffer_size);
    if (size_ <= stack_buffer_size) {
      size_ = stack_buffer_size;
    } else {
      size_ += +5 * (different_of_size_and_cap < 5) + minimumcapadd * (different_of_size_and_cap < minimumcapadd);
    }
  }
...
void *mjz_Str::realloc(void *ptr, size_t new_size) {
  ...
  if (ptr_Can_set_to_stack) {
    if (ptr_is_buffer) {
      ...
      if (the__length)
        memmove(stack_obj_buf.stack_buffer, ptr,
                min(the__length, stack_buffer_size));
    ...
    if (ptr_is_in_stack) {
      ...
      if (the__length) {
        memmove(ptr, stack_obj_buf.stack_buffer, min(the__length, stack_buffer_size));
      }
...
void mjz_Str::getBytes(unsigned char *buf, unsigned int bufsize,
  ...
  memcpy((char *)buf, buffer + index, min(n, len));
  buf[n] = 0;
  // (this->*update_event_F_p)();
}
...
mjz_Str &mjz_Str::ULL_LL_to_str_rep(uint64_t value, int radix, bool is_signed,
  ...
  reserve(70, 1);
  addto_length(
    strlen(b_U_lltoa(value, buffer_ref(), radix, is_signed, force_neg)), 1);
  return *this;
}
...
mjz_Str &mjz_Str::operator*=(unsigned int number_of_it) {
  ...
  for (int i{}; i < number_of_it; i++) { temperory_str += *this; }
...
unsigned long mjz_Str::UN_ORDERED_compare(const char *s, unsigned int s_len) const {
  ...
  for (long i{}; i < s_len; i++) {
    NUMBER_OF_EACH_char_array[0][ucs_[i]]++;
  }

  for (long i{}; i < this->length(); i++) {
    NUMBER_OF_EACH_char_array[1][ucbuffer_[i]]++;
  }

  for (long i{}; i < 256; i++) {
    number_of_not_right += abs((long long)NUMBER_OF_EACH_char_array[1][i] - (long long)NUMBER_OF_EACH_char_array[0][i]);
  }
...
hash_sha_512 hash_msg_to_sha_512_n(const char *dev_passwoed,
                                   ...
                                   String output_name) {  // intended  copy
  if (n == 0) {
