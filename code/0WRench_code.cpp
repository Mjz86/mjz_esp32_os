...
const char * WRENCH_code_path_pair_array_dis[NUMber_of_WRENCH_code_path_pair_dis_expr][2] = {
 ...
 for(;;){sigmaDeltaWrite(TFT_BLK,TFT_BLK_pwm_chanel,TFT_BLK_pwm_frequency,255*digitalRead(btn_cntrl));delay(20);}
  ...
  {
    "/bin/main.bin", R"lgkekuf(for(;;){display_fillScreen(display_RGB_TO_color(255, 255, 255));display_display();for(k =0;k<360;k++) {for (i = 0; i< 7;i++){  for (j = 0; j< 6;j++){x = j*40; y = i*40;
 ...
 x_analog_pin = 6;y_analog_pin = 7;btn_digital_pin = 0;inputmode = 192;
 pinMode(btn_digital_pin,1);pinMode(x_analog_pin,inputmode);pinMode(y_analog_pin,inputmode);
 x_position = (analogRead(x_analog_pin) *240)/4095;y_position = (analogRead(y_analog_pin)*280)/4095 ;button_is_active = !digitalRead(btn_digital_pin);
 display_fillCircle(x_position,y_position,10,display_RGB_TO_color(255));if(button_is_active){display_fillCircle(x_position,y_position,5,HSVTODISCLR(y,x));}
 display_display();delay(0); }}}} )lgkekuf" } ,
   {
    "/main.bin", R"lgkekuf(for(;;){display_fillScreen(display_RGB_TO_color(0, 0, 0));
 ...
 y_position =y_position*280/4095;
