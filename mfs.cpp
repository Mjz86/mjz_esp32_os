...
String get_src_folder_for_path(const String& path) {
  ...
  if (indexLST == path.indexOf("/")) return path.substring(0, indexLST + 1);
  if (indexLST == (path.length() - 1)) return get_src_folder_for_folder(path);
  return get_src_folder_for_file(path);
}
...
mjz_fs_cls::mjz_fs_cls()
  : fs__(LittleFS), root_json_doc_obj(32768) {
  if (!begined_fs) {
...
bool mjz_fs_cls::findNestedKey_rm_hlpr(const String& path, mod_important mod) {
  ...
  for (uint32_t i{}; i < root_json_str_bfr_obj.length(); i++) {
    ...
    while (1) {
      if (findex > 1)
        if ('\"' == root_json_str_bfr_obj[findex - 1]) break;
    ...
    if (root_json_str_bfr_obj[eindex - 1] != '/') {
      if (usr_sudo_chose(get_obj_json_fil_frm_pth(root_json_doc_obj, root_json_str_bfr_obj.substring(findex, eindex)), mod)["w"] == 0) return 1;
    }
    i = eindex;
  }
  return 0;
...
File mjz_fs_cls::open(const char* path, const char* mode, const bool create, mod_important mod) {
  ...
  if (create && !exists(path, dev)) {
    if (!exists(get_src_folder_for_path(path), dev)) mkdir(get_src_folder_for_path(path).c_str(), mod);
    set_permit_to_path(path, defultfprmt_null);
  }
  update_root_json_File();
  if (mode == "r") {
    if (mod_to_rwx_permit(mod, get_permit_from_path(path, 0)).r != 1) return File();
  } else if (mode == "x") {
    if (mod_to_rwx_permit(mod, get_permit_from_path(path, 0)).x != 1) return File();
    else {  // temperory  the real is the run function
      return fs__.open(path, "r", create);
    }
  } else {  //if (mode =="w") || (mode =="a")
    if (mod_to_rwx_permit(mod, get_permit_from_path(path, 0)).w != 1) return File();
  }
  update_root_json_File();
...
bool mjz_fs_cls::remove(const char* path, mod_important mod) {
  if (!(exists(path, mod))) return 0;
  if (!get_obj_json_fil_frm_pth(root_json_doc_obj, path).isNull()) {
    //file
    if (mod_to_rwx_permit(mod, get_permit_from_path(path, 0)).w != 1) return 0;
...
bool mjz_fs_cls::chmod(const char* path, filder_permit ch2mod, mod_important mod) {
  ...
  if (!exists(path, mod) || mod_to_rwx_permit(mod, oldmod_).w != 1) return 0;
...
String mjz_fs_cls::ls(const char* path, bool is_Pretty, mod_important mod) {
  update_root_json_File();
  String ret = "{";
  String root_json_str_bfr_obj;
  serializeJson(root_json_doc_obj, root_json_str_bfr_obj);
  for (uint32_t i{}; i < root_json_str_bfr_obj.length(); i++) {
    ...
    if (
      ...
      root_json_str_bfr_obj[eindex - 1] != '/' && get_src_folder_for_path(root_json_str_bfr_obj.substring(findex, eindex)) == path) {
    ...
    } else if (root_json_str_bfr_obj[eindex - 1] == '/' && get_src_folder_for_path(root_json_str_bfr_obj.substring(findex, eindex)) == path && root_json_str_bfr_obj.substring(findex, eindex) != path) {
  ...
  if (mod == mod_important::usr) {
    for (JsonPair kv : bufr__.as<JsonObject>()) {
      if (kv.value()["usr"]["r"].isNull()) continue;
      if (kv.value()["usr"]["r"] == 0) bufr__.remove(kv.key().c_str());
    }
  }
  if (mod == mod_important::sudo) {
    for (JsonPair kv : bufr__.as<JsonObject>()) {
      if (kv.value()["sudo"]["r"].isNull()) continue;
      if (kv.value()["sudo"]["r"] == 0) bufr__.remove(kv.key().c_str());
    }
  }
...
bool mjz_fs_cls::rmdir(const char* path, mod_important mod) {
  ...
  if (get_obj_json_fldr_frm_pth(root_json_doc_obj, path) == 1) {
    ...
    update_root_json_File();
    {
      if (findNestedKey_rm_hlpr(path, mod)) return 0;
    }
    String root_json_str_bfr_obj;
    serializeJson(root_json_doc_obj, root_json_str_bfr_obj);
    for (uint32_t i{}; i < root_json_str_bfr_obj.length(); i++) {
...
bool mjz_fs_cls::rm(const char* path, mod_important mod) {
  if (path[strlen(path) - 1] == '/') return rmdir(path, mod);
