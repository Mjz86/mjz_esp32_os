...
namespace mjz_ard {
  ...
  bool mjz_fs_cls::exists( const char * path, mod_important mod ) {
    ...
    if ( fs__.exists( path ) && !get_obj_json_fil_frm_pth( root_json_doc_obj, path ).isNull() && mod_to_rwx_permit( mod, get_permit_from_path( path, 0 ) ).r == 1 ) {
      ...
      if ( mod_to_rwx_permit( mod, get_permit_from_path( path, 0 ) ).w == 1 && mod_to_rwx_permit( mod, get_permit_from_path( path, 0 ) ).r == 1 ) {
  ...
  String mjz_fs_cls::ls( const char * path, bool is_Pretty, mod_important mod ) {
    ...
    for ( uint32_t i{ }; i < root_json_str_bfr_obj.length(); i++ ) {
      ...
      if (
        ...
        if ( root_json_str_bfr_obj[eindex - 1] == '/' && get_src_folder_for_path( root_json_str_bfr_obj.substring( findex, eindex ) ) == path && root_json_str_bfr_obj.substring( findex, eindex ) != path ) {
