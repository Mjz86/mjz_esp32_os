...
namespace mjz_ard {
  ...
  uint16_t makeWord( uint8_t h, uint8_t l ) {
    return ( h << 8 ) | l; } } // namespace mjz_ard
#endif
/*
...
#define null_char 0
  ...
  mjz_Str ULL_LL_to_str( uint64_t value, int radix, bool is_signed,
    ...
    ret_var.addto_length(
      strlen( b_U_lltoa( value, ( char * )ret_var, radix, is_signed, force_neg ) ) );
    return ret_var; }
...
#define min(a, b) \
  ...
  bool mjz_Str::reserve( unsigned int size_ ) {
...
#define PGM_P const char *
    ...
    for ( ;; i++ ) {
      ...
      if ( is_reinterpreted == is_reinterpreted_and_is_int ) {
        ...
        if ( is_reinterpreted == 1 ) {
          ...
          if ( rhs.drived_mjz_Str_DATA_storage_Obj_ptr->reinterpret_char_char == bfr[i] ) {
            bfr[i] = '\0';
            i--;
            is_reinterpreted = 1;
            continue; }
  ...
  mjz_Str & mjz_Str::ULL_LL_to_str_rep( uint64_t value, int radix, bool is_signed,
    ...
    reserve( 70 );
    addto_length(
      strlen( b_U_lltoa( value, buffer_ref(), radix, is_signed, force_neg ) ) );
    return *this; }
