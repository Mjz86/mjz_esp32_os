...
for (uint i = 0; i < msglen; i++) { Data_buffer.out_message_frame[i] = 0; }
 ...
 memcpy(Data_buffer.out_message_frame, (const char *)&Data_buffer.output_HASH.Hashed[0], sizeof(Data_buffer.output_HASH));
  ...
  for (uint i = 0; i < msglen; i++) { Data_buffer.out_message_frame[i] = 0; }
  Data_buffer.out_message_frame[msglen] = 0;  // null termination
  if (min(out_str.available(), msglen)) {memcpy(Data_buffer.out_message_frame,out_str.c_str(), min(out_str.available(), msglen));
...
bool Str_ECC_msger_class::setup_input_string() {
 ...
 if(input_length > input_vector.size()*msglen) return  0;
