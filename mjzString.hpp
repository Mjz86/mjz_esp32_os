...
// The string class
#ifndef Arduino
class mjz_Str : public std__string_view_if_is, public Stream {
#else
class mjz_Str : public Stream {
#endif  //Arduino
...
public:
  ...
  if_virtual_then_virtual size_t write(const char *buf, size_t size);
  if_virtual_then_virtual size_t write(const char *buf);
  if_virtual_then_virtual size_t write(uint8_t) override;
  if_virtual_then_virtual size_t write(const uint8_t *buf, size_t size) override;
  ...
  if_virtual_then_virtual size_t read(uint8_t *buf, size_t size);
  ...
  mjz_Str(std__string_view_if_is &x) {
    if (std::string(x).c_str() != 0)
      mjz_Str(std::string(x).c_str());
    else
      mjz_Str("");
  }
  mjz_Str(std__string_view_if_is &&x) {
  ...
  if_virtual_then_virtual mjz_Str &operator=(std__string_view_if_is &&x) {
    ...
    if_virtual_then_virtual bool reserve(unsigned int size);
    inline unsigned int length(void) const {
      return len;
    }
    bool addto_length(uint32_t addition_tolen) {
      bool ret = 1;
      if ((addition_tolen + len) > capacity) ret = reserve(addition_tolen + len + 1);
      if (ret) len += addition_tolen;
      return ret;
    }
    ...
    if_virtual_then_virtual explicit operator const uint8_t *() {
    ...
    if_virtual_then_virtual explicit operator const void *() {
      return (void *)buffer_ref();
    }
    if_virtual_then_virtual explicit operator const uint16_t *() {
      return (uint16_t *)buffer_ref();
    }
    if_virtual_then_virtual explicit operator const uint32_t *() {
      return (uint32_t *)buffer_ref();
    }
    if_virtual_then_virtual explicit operator const uint64_t *() {
      return (uint64_t *)buffer_ref();
    }
    ...
    /*       if_virtual_then_virtual explicit operator char [] () {
             ...
             if_virtual_then_virtual explicit operator const char [] () {
              return buffer_ref();
            } */
    ...
    if_virtual_then_virtual explicit operator const std__string_view_if_is() const {
      return std__string_view_if_is((const char *)buffer_ref());
    }
    ...
    // return new std::string ( ( const char* ) buffer_ref() );
    // }
    ...
    const char the_in_reinterpreted_char_cca[15] = { 'n', 'r', 'a', 'b', 't', 'v', 'f', 'e'  // world wide
                                                     ,
                                                     's', '@', '\\', '\"', '\'', '\0' };              // just me
    const char the_out_reinterpreted_char_cca[15] = { '\n', '\r', '\a', '\b', '\t', '\v', '\f', '\e'  // world wide
                                                      ,
                                                      ' ', '@', '\\', '\"', '\'', '\0' };  // just me
  protected:
    std::string the_in_reinterpreted_char = the_in_reinterpreted_char_cca;
    std::string the_out_reinterpreted_char = the_out_reinterpreted_char_cca;
    char reinterpret_char_char = '\\';
  public:

    if_virtual_then_virtual bool add_to_reinterpreted_char(char, char);
    if_virtual_then_virtual bool remove_from_reinterpreted_char(char);
    if_virtual_then_virtual bool change_reinterpret_char_char(char);

    if_virtual_then_virtual bool char_to_char_for_reinterpret(char &c_char) {
      if (c_char == reinterpret_char_char)
        return reinterpret_char_char;
      uint32_t i{};
      while (1) {
        if (the_in_reinterpreted_char.c_str()[i] == c_char) {
          c_char = the_out_reinterpreted_char.c_str()[i];
          return 1;
        } else if (the_in_reinterpreted_char.c_str()[i] == '\0')
          return 0;
        i++;
      }
    }

    if_virtual_then_virtual int8_t char_to_int_for_string(char c_char) {
      if (c_char < '0' || c_char > '9') return -1;
      return (int8_t)(c_char - '0');
    }
    friend std::istream &helper__op_shift_input_(mjz_Str & rhs, std::istream & CIN, mjz_Str & get_shift_op_s) {
      char bfr[2050];
    ...
    if_virtual_then_virtual void toCharArray(char *buf, unsigned int bufsize, unsigned int index = 0) const {
      getBytes((unsigned char *)buf, bufsize, index);
    }
    if_virtual_then_virtual char *&buffer_ref() {
      if ((len + 1) < capacity) reserve(capacity + 5);
      operator[](length()) = '\0';
      return buffer;
    }
    if_virtual_then_virtual const char *buffer_ref() const {
      return buffer;
    }
    if_virtual_then_virtual const char *c_str() const {
      return buffer_ref();
    }

    if_virtual_then_virtual const char *c_str() {
      return buffer_ref();
    }
    if_virtual_then_virtual char *begin() {
      return buffer_ref();
    }
    if_virtual_then_virtual char *end() {
      return buffer_ref() + length();
    }
    if_virtual_then_virtual char *endAST() {
      return buffer_ref() + length();
    }
    if_virtual_then_virtual const char *begin() const {
      return c_str();
    }
    if_virtual_then_virtual const char *endAST() const {
      return c_str() + length();
    }
    if_virtual_then_virtual const char *end() const {
      return c_str() + length();
    }
    ...
    if_virtual_then_virtual std::string &change_forbiden();

    if_virtual_then_virtual bool is_forbiden(char);
  ...
  protected:
    const char forbiden_chars_cnt[12] = {  3, 4, 23, 24, 25, 27, 28, 29, 30, 31, 127, 0 };
    std::string forbiden_chars = forbiden_chars_cnt;
    void (mjz_Str::*update_event_F_p)();  //to call (object_ptr->*pointer_name)(arguments)
    ...
    /* #else
        ...
        // i khow the *& is  cursed
        char *& buffer = std__string_view_if_is::buffer; // the actual char array
        unsigned int & capacity = std__string_view_if_is::capacity; // the array length minus one (for the '\0')
        unsigned int & len = std__string_view_if_is::len; // the mjz_Str length (not counting the '\0')
...
//void operator >> ( mjz_Str & rhs,std::istream & ) ;

/*
    std::istream & operator >> ( std::istream& CIN,StringSumHelper & rhs ) = delete;
    void operator >> ( StringSumHelper & rhs,std::istream & ) = delete;

    std::istream & operator >> ( std::istream& CIN,const mjz_Str & rhs ) = delete;
    std::istream & operator >> ( std::istream& CIN,const StringSumHelper & rhs ) = delete;
    void operator >> ( const mjz_Str &,std::istream & ) = delete;
    void operator >> ( const StringSumHelper &,std::istream & ) = delete;
  */
